...
const log = new _zosLib.Logger('scripts/status');

exports.default = async function status({ network, txParams = {}, networkFile = undefined }) {
  const controller = (0, _ControllerFor2.default)(network, txParams, networkFile);
...
async function contractsInfo(controller) {
  ...
  _lodash2.default.each(controller.packageFile.contracts, function (contractName, contractAlias) {
  ...
  controller.networkFile.contractAliasesMissingFromPackage().forEach(contractAlias => log.warn(`- ${contractAlias} will be removed on next push`));
}
...
async function dependenciesInfo(networkFile) {
  ...
  _lodash2.default.forEach(packageFile.dependencies, (requiredVersion, dependencyName) => {
  ...
  _lodash2.default.forEach(networkFile.dependenciesNamesMissingFromPackage, dependencyName => {
