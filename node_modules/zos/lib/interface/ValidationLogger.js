...
const log = new _zosLib.Logger('Validations');
...
class ValidationLogger {
  ...
  log(validations, buildArtifacts = undefined) {
    ...
    this.logStorageLayoutDiffs(storageDiff, (0, _zosLib.getStorageLayout)(this.contract, buildArtifacts));
  }
  ...
  logUninitializedBaseContracts(uninitializedBaseContracts) {
    if (!_lodash2.default.isEmpty(uninitializedBaseContracts)) {
  ...
  logUncheckedVars(vars) {
    if (_lodash2.default.isEmpty(vars)) return;
    ...
    log.warn(`- ${variablesString} ${varList} ${containsString} a struct or enum. These are not automatically checked for storage compatibility in the current version. ` + `See ${STORAGE_CHECKS_LINK} for more info.`);
  }

  logStorageLayoutDiffs(storageDiff, updatedStorageInfo) {
    if (_lodash2.default.isEmpty(storageDiff)) return;
    const originalTypesInfo = this.existingContractInfo.types || {};

    storageDiff.forEach(({ updated, original, action }) => {
      const updatedSourceCode = updated && _zosLib.FileSystem.exists(updated.path) && _zosLib.FileSystem.read(updated.path);
      ...
      const updatedVarDescription = updated && (_tryGetSourceFragment(updatedSourceCode, updatedVarType.src) || [updatedVarType.label, updated.label].join(' '));
      ...
      switch (action) {
        case 'insert':
          log.error(`- New variable '${updatedVarDescription}' was inserted in contract ${updated.contract} in ${updatedVarSource}. ` + `You should only add new variables at the end of your contract.`);

          break;
        case 'delete':
          log.error(`- Variable '${originalVarDescription}' was removed from contract ${original.contract}. ` + `You should avoid deleting variables from your contracts.`);
          break;
        case 'append':
          log.info(`- New variable '${updatedVarDescription}' was added in contract ${updated.contract} in ${updatedVarSource} ` + `at the end of the contract.`);
          break;
        case 'pop':
          log.warn(`- Variable '${originalVarDescription}' was removed from the end of contract ${original.contract}. ` + `You should avoid deleting variables from your contracts.`);
          break;
        case 'rename':
          log.warn(`- Variable '${originalVarDescription}' in contract ${original.contract} was renamed to ${updated.label} in ${updatedVarSource}.` + `${updated.label} will have the value of ${original.label} after upgrading.`);
          break;
        case 'typechange':
          log.warn(`- Variable '${original.label}' in contract ${original.contract} was changed from ${originalVarType.label} ` + `to ${updatedVarType.label} in ${updatedVarSource}. Avoid changing types of existing variables.`);
          break;
        case 'replace':
          log.warn(`- Variable '${originalVarDescription}' in contract ${original.contract} was replaced with '${updatedVarDescription}' ` + `in ${updatedVarSource}. Avoid changing existing variables.`);
          break;
        default:
