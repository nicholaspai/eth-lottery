...
const log = new _zosLib.Logger('StatusFetcher');
...
class StatusFetcher {
  ...
  async onMissingRemoteImplementation(expected, observed, { alias, address }) {
    ...
    const buildPath = _zosLib.Contracts.getLocalPath(contractName);
    if (_zosLib.FileSystem.exists(buildPath)) {
      const contract = _zosLib.Contracts.getFromLocal(contractName).at(address);
      const remoteBodyBytecode = (await (0, _util.promisify)(web3.eth.getCode.bind(web3.eth))(address)).replace(/^0x/, '');
      const bodyBytecodeHash = (0, _zosLib.bytecodeDigest)(remoteBodyBytecode);
      if ((0, _zosLib.bodyCode)(contract) === remoteBodyBytecode) {
        log.warn(`Assuming that constructor function of local version of ${contractName} is the one registered`);
        const constructor = (0, _zosLib.constructorCode)(contract);
        const bytecodeHash = (0, _zosLib.bytecodeDigest)(`${constructor}${remoteBodyBytecode}`);
  ...
  onMismatchingImplementationAddress(expected, observed, { alias, address }) {
    ...
    this.networkFile.updateImplementation(alias, implementation => _extends({}, implementation, { address }));
  }
  ...
  onMismatchingImplementationBodyBytecode(expected, observed, { alias, address, bodyBytecodeHash }) {
    ...
    this.networkFile.updateImplementation(alias, implementation => _extends({}, implementation, { bodyBytecodeHash }));
  }
  ...
  onMismatchingProxyImplementation(expected, observed, { packageName, address, version, implementation, alias }) {
    ...
    this.networkFile.updateProxy({ package: packageName, contract: alias, address }, proxy => _extends({}, proxy, { implementation: observed }));
  }
  ...
  onMismatchingDependencyAddress(expected, observed, { name, address }) {
    ...
    this.networkFile.updateDependency(name, dependency => _extends({}, dependency, { package: observed }));
  }
  ...
  onMismatchingDependencyVersion(expected, observed, { name, version }) {
    ...
    this.networkFile.updateDependency(name, dependency => _extends({}, dependency, { version: observed }));
  }
