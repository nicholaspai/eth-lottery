...
class StatusChecker {
  ...
  async setProject() {
    try {
      ...
      if (!this._project) {
        this._project = this.networkFile.isLib ? await _zosLib.LibProject.fetchOrDeploy(this.networkFile.version, this.txParams, { packageAddress }) : await _zosLib.AppProject.fetchOrDeploy(this.packageName, this.networkFile.version, this.txParams, { appAddress, packageAddress });
      }
  ...
  async checkVersion() {
    ...
    if (!(0, _zosLib.semanticVersionEqual)(observed, expected)) this.visitor.onMismatchingVersion(expected, observed);
  }
  ...
  async checkImplementations() {
    ...
    await Promise.all(implementationsInfo.map(async info => {
      const { address } = info;
      const bytecode = await (0, _util.promisify)(web3.eth.getCode.bind(web3.eth))(address);
      return await ((0, _zosLib.isSolidityLib)(bytecode) ? this._checkRemoteSolidityLibImplementation(info, bytecode) : this._checkRemoteContractImplementation(info, bytecode));
    }));
  ...
  _checkContractImplementationBytecode(alias, address, bytecode) {
    ...
    const observed = (0, _zosLib.bytecodeDigest)(bytecode);
  ...
  _checkSolidityLibImplementationBytecode(alias, address, bytecode) {
    ...
    const observed = (0, _zosLib.bytecodeDigest)((0, _zosLib.replaceSolidityLibAddress)(bytecode, address));
    if (observed !== expected) this.visitor.onMismatchingImplementationBodyBytecode(expected, observed, { alias, address, bodyBytecodeHash: observed });
  }
  ...
  _checkRemoteProxy(remoteProxyInfo) {
    ...
    if (localProxyInfo) {
      ...
      this.visitor.onMissingRemoteProxy('none', 'one', _extends({}, remoteProxyInfo, { packageName: this.packageName }));
    }
  }
  ...
  _checkProxyAlias(localProxyInfo, remoteProxyInfo) {
    ...
    if (observed !== expected) this.visitor.onMismatchingProxyAlias(expected, observed, _extends({ packageName, version }, remoteProxyInfo));
  }
  ...
  _checkProxyImplementation(localProxyInfo, remoteProxyInfo) {
    ...
    if (observed !== expected) this.visitor.onMismatchingProxyImplementation(expected, observed, _extends({ packageName, version }, remoteProxyInfo));
  }

  _checkUnregisteredLocalProxies(proxiesInfo) {
    const foundAddresses = proxiesInfo.map(info => info.address);
    this.networkFile.getProxies().filter(proxy => !foundAddresses.includes(proxy.address)).forEach(proxy => {
  ...
  _checkDependencyVersion(name, version) {
    ...
    if (!(0, _zosLib.semanticVersionEqual)(version, expected)) this.visitor.onMismatchingDependencyVersion(expected, version, { name, version });
  }
  ...
  _checkUnregisteredLocalDependencies(dependenciesInfo) {
    ...
    this.networkFile.dependenciesNames.filter(name => !foundDependencies.includes(name)).forEach(name => {
      const dependency = this.networkFile.getDependency(name);
      this.visitor.onUnregisteredDependency('one', 'none', _extends({}, dependency, { name }));
    });
  }
  ...
  async _fetchOnChainImplementations() {
    ...
    const events = allEvents.filter((event, index) => contractsAlias.lastIndexOf(event.args.contractName) === index).filter(event => event.args.implementation !== ZERO_ADDRESS).map(event => ({ alias: event.args.contractName, address: event.args.implementation }));
  ...
  async _fetchOnChainPackages() {
    ...
    const filteredEvents = allEvents.filter(event => event.args.package !== ZERO_ADDRESS).filter(event => event.args.providerName !== this.packageName).map(event => ({
